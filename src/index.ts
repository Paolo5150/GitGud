      ...
      submenu: [
        {
          label: 'Branch Finder',
          click: async ()=>{         
            OpenBrancheFinderDialog(MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY)
          }
        }]
    },
...
ipcMain.on('clicked-add-repo', async (event) =>
  {
    ...
    try{
        ...
        if (!result.canceled) {
          ...
          event.reply('confirm-repo-name', selectedFolderPath)
      }
...
ipcMain.on('clicked-find-branch', async (event, branchName:string, list:Array<string>) =>
  {
    var current = GetCurrentRepoPath();
